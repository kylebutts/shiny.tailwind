...
In your shiny UI declaration, just include `shiny.tailwind::use_tailwind` and all the appropraite files will be inserted into your shiny app. Therefore you can just start using tailwind classes and they will load dynamically and automatically. 
...
For version 2, if you want to use custom modules, for example TailwindTypography, note that you need to use the browser-version and you have to layout the config file in a specific way. You need to define the config JSON object as 'window.tailwindConfig' and you must call 'window.tailwindCSS.refresh();'. An example is in the folder 'inst/examples/03-modules' in the github repository. 
