...
If you want to compile your used tailwindcss classes to a local css file, see the [Details](#details) section.
...
## Details about `shiny.tailwind` {#details}
...
See also `?install_tailwindcss_cli()` to install the CLI (the program is around 15MB and platform dependent), `?compile_tailwindcss()` to compile the files to a local CSS, or the `03-css-generation` example (`system.file("examples", "03-css-generation", package = "shiny.tailwindcss")`).
